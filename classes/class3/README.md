...
To get Express to render handlebars, as well as to add some useful templating features which we will get into later, we are going to use the [Express-Handlebars](https://github.com/ericf/express-handlebars) module. So go ahead and `npm --save install express-handlebars` to get it installed.

Now two more things to know about are layouts and partials.
* Layouts: Basically meta-templates. They are handlebars documents that allow you to create a wrapper around all the templates that use them.
* Partials: Basically mini-reusable templates. They are handlebars documents that can be written once and "included" within other templates. (ei. a navigation bar that appears the same on every page)
...
```
This is some of the boiler plate for HTML with the center bit `{{{body}}}`. That signifies where templates will be inserted into this layout.
...
```
All of this organization is completely customizable, but we'll leave it as the defaults for now.

Now let's hook it all up!

Add the line `var exphbs  = require('express-handlebars');` to the top of your `app.js` file. Then add these two lins after the `var app = express();` line:
```javascript
